...
import { Gamepad2, Globe, Search, Plus, Loader2, Trash2, Clock, MapPin, Users, Play, FolderOpen, Download } from 'lucide-react';
...
export default function GamesPage({ category }: GamesPageProps) {
    ...
    const headerIcon = isOnline ? (
        <Globe className="w-6 h-6 text-white" />
    ) : (
        <Gamepad2 className="w-6 h-6 text-white" />
    );

    const headerTitle = isOnline ? 'Online Games' : 'Offline Games';
    const headerSubtitle = isOnline ? 'Join multiplayer matches worldwide' : 'Manage your local game library';
    const createButtonText = isOnline ? 'Create Lobby' : 'Add Game';

    // --- Data Fetching ---
    const { data, isLoading } = useQuery({
        queryKey: ['games', category, searchQuery],
    ...
    const joinMutation = useMutation({
        ...
        onSuccess: (data, variables) => {
            const path = category === 'ONLINE' ? 'online' : 'offline';
            router.push(`/games/${path}/${variables}`);
            queryClient.invalidateQueries({ queryKey: ['games'] });
        },
        onError: (error: any, variables) => {
            ...
            if (error?.response?.data?.message?.toLowerCase().includes('already joined') ||
                error?.message?.toLowerCase().includes('already joined')) {
                router.push(`/games/${path}/${variables}`);
                return;
            }
    ...
    const handleJoin = (e: React.MouseEvent, gameId: string) => {
        e.stopPropagation();
        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-gray-800 tracking-tight">{headerTitle}</h1>
            ...
            {!isLoading && games.length === 0 && (
                isOnline ? (
                    ...
                    <div className="bg-white/60 border-2 border-dashed border-gray-300 rounded-2xl p-12 text-center">
                        ...
                        <p className="text-gray-600">Your offline game library is empty.</p>
                    </div>
                )
            )}
                    ...
                    >
                        <div>
                            ...
                            <div className="space-y-3 mb-5">
                                ...
                                {/* Player Count (Unified) */}
                                <div className="flex items-center gap-2 text-gray-500 font-medium text-sm">
                                    <Users className="w-4 h-4 text-purple-500" />
                                    <span>
                                        <span className="text-gray-900 font-bold">{game.currentPlayers || 0}</span>/{game.maxPlayers} Players
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="mb-6">
                                <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                                    <div
                                        ...
                                        style={{ width: `${((game.currentPlayers || 0) / game.maxPlayers) * 100}%` }}
                                    ></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-2">
                            {(() => {
                                ...
                                if (isOnline) {
                                    return (
                                        <button
                                            onClick={(e) => isJoined ? router.push(`/games/online/${game._id}`) : handleJoin(e, game._id)}
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {isJoined ? 'Resume Game' : 'Play Game'}
                                        </button>
                                    );
                                }
